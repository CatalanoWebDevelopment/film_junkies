...
class MoviePage extends Component { 
    ...
    render() {
        if (this.state.moveInfo === "" || this.state.configuration === "") {
            // Loading Screen
            return (
            ...
            const { original_title, overview, release_date, runtime } = this.state.movieInfo; 
            const base_url = this.state.configuration.images.base_url;

            const RuntimeDisplay = () => this.getRuntimeInHoursAndMinutes(runtime);
            const ratingPercent = this.state.movieInfo.vote_average * 10;
            const yearDisplay = release_date;
